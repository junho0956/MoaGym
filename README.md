...
- 21.04.26
    ...
    - fix css styling
    - 나는 바보가 아닐까에 대한 기록
        loading 컴포넌트를 띄워준 후에 이미지가 onload 되면 본 컴포넌트를 띄워주기 위한 작업을 처리하고 있었는데
        계속해서 미리 작업해둔 onload 속성을 입힌 변수가 제대로 로드되지 않았다.
        계속된 디버깅 끝에 알게된 사실은.. onload 속성을 입힌 변수가 훅 내부에서 일반적으로 선언한 상수였기 떄문에...
        뜬금없이 한번씩 이런짓을 하는데 오늘은 정말 크게 시간을 버리고 말았다.
        useRef를 이용하여 문제를 해결할 수 있었다.. 오늘 시간 버린만큼 다음부터는 좀 더 집중해서 코딩하자..

- 21.04.21
    - infinity scroll 문제점에 대한 고민..
        배포하고 나서 생각해보니 infinity scroll을 잘못 구현했다는 것을 깨달았다.
        애초에 개발 자체를 내가 보고있는 화면을 기준으로 코드를 진행했기 때문이다.
        고정된 컴포넌트 갯수를 사용하면 각기 다른 사용자의 clientHeight에 대한 대처를 진행할 수 없다.
